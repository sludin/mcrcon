...
static int global_multiple_commands = 0;
...
int main(int argc, char *argv[])
{
        ...
        opterr = 1;
        int opt;
        while ((opt = getopt(argc, argv, "vrtcshw:H:p:P:m")) != -1)
        {
                switch (opt) {
                        ...
                        case 'm': global_multiple_commands = 1; break;
                        case 'w':
...
int run_commands(int argc, char *argv[])
{
        ...
        else
        {
                ...
                *p = 0;
                ...
                if (!rcon_command(global_rsock, command)) {
                        return EXIT_FAILURE;
                }
                else {
                        return EXIT_SUCCESS;
...
char *command_generator( const char *text, int state )
{
    ...
    const char **commands = get_command_list();
...
char **command_completion( const char *text, int start __attribute__((unused)), int end __attribute__((unused)) )
{
        ...
        /* If start == 0 then this is a base command.  Complete it otherwise do not.*/
        if ( start == 0 ) {
